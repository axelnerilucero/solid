...
contract ArtToken is ERC721, Ownable {
    ...
    // Pricing of NFT tokenss (price of the artwork)
    uint256 public fee = 5 ether;

    // Data structurw with the propoierties of the artrork
    struct Art {
        string name;
        uint256 id;
        uint256 dna;
        ...
        //uint256 price;   // Price of the art, in wei (e.g., 10^9 wei is one eth)
    }
    ...
    // Creation of a random number (requires for NFT token propierties)
    function _createRandomNum(uint256 _mod) internal view returns (uint256){
        bytes32 hash_randonNumb = keccak256(abi.encodePacked(block.timestamp, msg.sender));
        uint256 randomNum = uint256(hash_randonNumb);
    ...
    function infoSmartContract() public view returns(address, uint256){
        ...
        uint256 SC_mooney = address(this).balance/10**18;
        return (SC_address, SC_mooney);
    }
    ...
    function createArtWork(string memory _name) public payable {
        require(msg.value >= fee);
        _createArtWork(_name);
    }

    // Extraction of ethers from the 
